...
export class contactService {
    ...
    static firebaseURL = 'https://vue-contact-tra-default-rtdb.asia-southeast1.firebasedatabase.app/'
    ...
    static async getContact(id) {
        const response = await axios.get(this.firebaseURL + '/contacts/' + id + '.json')
        const responseData = await response.data

        if (response.status !== 200) {
            throw new Error('Unable to view the selected contact! Please try again later.')
        }
    ...
    static async addNewContact(formData) {
        const response = await axios.post(this.firebaseURL + '/contacts.json', formData)
    ...
    static async updateContact(id, formData) {
        const response = await axios.put(this.firebaseURL + '/contacts/' + id + '.json', formData)

        if (response.status !== 200) {
            throw new Error('Unable to update the selected contact! please try again later.')
        } else {
    ...
    static async deleteContact(id) {
        const response = await axios.delete(this.firebaseURL + '/contacts/' + id + '.json')
